...
<template>
	<form ref="loginForm" class="login-form" method="post" name="login" :action="loginActionUrl" @submit="submit">
		<fieldset class="login-form__fieldset" data-login-form>
			...
			<h2 class="login-form__headline" data-login-form-headline>
				{{ headlineText }} <br>
				<span class="subtitle">Sign in to your account to continue.</span>
			</h2>
			<NcTextField id="user" ref="user" :label="loginText" name="user" placeholder=" " :maxlength="255"
				:value.sync="user" :class="{ shake: invalidPassword }" class="custom-form-login" autocapitalize="none"
				:spellchecking="false" :autocomplete="autoCompleteAllowed ? 'username' : 'off'" required
				:error="userNameInputLengthIs255" :helper-text="userInputHelperText" data-login-form-input-user
				@change="updateUsername" />

			<NcPasswordField id="password" ref="password" name="password" placeholder=" "
				:class="{ shake: invalidPassword }" class="custom-form-login" :value.sync="password"
				:spellchecking="false" autocapitalize="none"
...
export default {
	...
	setup() {
		...
		return {
			...
			headlineText: t('core', 'Log in', { productName: OC.theme.name }, undefined, { sanitize: false, escape: false }),
...
::v-deep .input-field__helper-text-message {
